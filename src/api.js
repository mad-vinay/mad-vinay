...
const saveTransactionDetails = params => {
  const URL = "/v1/tigo/saveTransactionDetails";
  // return axios.post(`http://localhost:5050/${URL}`, params).then(
  return axios.post(apiURL(URL), params).then(
    resp => {
      return resp
    },
    e => {
...
const fetchThresholdValue = msisdn => {
  // debugger
  const URL = "/v1/tigo/userMinimumBalance/" +msisdn;
  return axios.get(apiURL(URL)).then(
    resp => {
      return resp
    },
    err => Promise.reject(new Error('Failed'))
  )
}
